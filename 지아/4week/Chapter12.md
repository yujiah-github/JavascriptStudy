...
- 스택 오버 플로 에러를 발생시킬 우려가 있으므로, 재귀함수는 반복문을 사용하는 것보다 재귀 함수를 사용하는 편이 더 직관적으로 이해하기 쉬울 때만 한정적으로 사용하는 것이 바람직하다.

#### 7.3 중첩 함수
- 함수 내부에 정의된 함수를 중첩 함수 또는 내부 함수라고 한다.
- 중첩 함수를 포함하는 함수는 외부 함수라고 부르고, 중첩 함수는 자신을 포함하는 외부 함수를 돕는 헬퍼 함수의 역할을 한다.
...
- 콜백 함수는 비동기 처리 뿐만 아니라 배열 고차 함수에서도 사용되는데, 자바스크립트에서 배열은 사용빈도가 높기 때문에 배열 고차 함수를 잘 아는 것이 중요하다.
